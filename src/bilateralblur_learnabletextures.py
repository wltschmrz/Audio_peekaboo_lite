...
def shifted_image(image: torch.Tensor, dx: int, dy: int) -> torch.Tensor:
    'image는 torch image로, 경계는 nan으로 패딩하여 경계 밖 픽셀을 추적하고 edge-case를 처리. Augmentation?'
    C_, H, W = image.shape
...
class LearnableImageFourier(LearnableImage):
    ...
    def __init__(self, 
        ...
        super().__init__(height, width, num_labels)
...
class LearnableImageFourierBilateral(LearnableImageFourier):
    ...
    def __init__(self,
        ...
        _, height, width = bilateral_blur.image.shape
        super().__init__(height, width, num_labels, hidden_dim, num_features, scale)
